...
if(window.parent.money.num>=0){
  ...
  window.parent.document.querySelector(".money").setAttribute("checked",isOpen);
...
function loadNum(){
  ...
  var fistDay = new Date('2018-05-14');
  var now = new Date();
  var timeDiff = Math.abs(now.getTime() - fistDay.getTime());
...
function makeEr(message){
    ...
    url = encodeURI(url);
    if(mobilecheck()){
        qrdiv.innerHTML = '<div class="barcode"><a href="'+url+'" class="alipay-button" target="_blank">'+config.mtext+'</a></div>';
    }else{
        qrdiv.innerHTML = '<div class="barcode"><img class="alypay-icon" src="https://static.clewm.net/cli/images/logomb/version2/alipay.png"/></div>'+config.text;
